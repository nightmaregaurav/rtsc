...
export class RelationalClassSpecificationBuilder<T extends PlainObject> {
  ...
  useTableName(tableName: string): RelationalClassSpecificationBuilder<T> {
  ...
  withIdentifier(identifier: PotentialIdentifierTypesIn<T>): RelationalClassSpecificationBuilder<T> {
    ...
    this.specification.isIdentifierString = typeof this.specification.registeredClass.prototype[identifier] === "string";
    return this;
  }

  hasOne<TT extends RelationalClassesIn<T>>(property: SingularRelationalPropertiesIn<T>, relatedClass: ClassReference<TT>, foreignKeyProperty: PotentialIdentifierTypesIn<T>): RelationalClassSpecificationBuilder<T> {
    if (this.specification.relationalProperties.find(x => x.name === property)) {
    ...
    this.specification.relationalProperties.push({
      ...
      idPropName: foreignKeyProperty as string,
      isList: false
    });
    return this;
  }

  hasMany<TT extends RelationalClassesIn<T>>(property: CollectionRelationalPropertiesIn<T>, relatedClass: ClassReference<TT>, foreignKeyProperty: PotentialIdentifierTypesIn<TT>): RelationalClassSpecificationBuilder<T> {
    if (this.specification.relationalProperties.find(x => x.name === property)) {
      throw new Error(`Cannot set multiple relational properties with the same name: ${property as string}`);
    }
    this.specification.relationalProperties.push({
  ...
  build(): RelationalClassSpecification<T> {
    if (!this.specification.identifier) {
    ...
    const schema = Object.keys(
      ...
      x => !this.specification.relationalProperties.find(y => y.name === x)
    );
    
    this.specification.schema = schema;
    this.createAndValidateSchema(schema, this.specification.tableName).then();
    return this.specification;
  }

  private async createAndValidateSchema(schema: string[], tableName: string): Promise<void> {
